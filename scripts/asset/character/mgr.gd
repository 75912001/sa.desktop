...
# 角色资源管理器.
# 当前桌面版直接使用 `assets/character/{id}.png`, `{id}.tpsheet` 和角色 offsets 总表.
# 这个类只负责建立“资源索引”和“帧播放元数据”, 不负责读取 character.yaml, 也不负责真正加载 Texture2D.
# Godot 的 RefCounted 不进入场景树, 因此这里没有 _ready() 生命周期; ConfigManager 创建 AssetManager 后会主动调用 load().
# 资源加载顺序固定为 PNG -> offsets -> .tpsheet:
# 1. PNG 文件名决定有哪些角色资源存在.
...
class Entry extends RefCounted:
	...
	# 动画构建器真正需要显示角色时, 才会通过这个路径 load() Texture2D, 避免启动阶段一次性加载全部图集.
	var png_path: String
...
func load() -> void:
	...
	var dir := DirAccess.open(Constants.ASSET_CHARACTER_DIR)
	if dir == null:
		assert(false, "角色资源目录不存在或无法打开: %s" % Constants.ASSET_CHARACTER_DIR)
		return
	...
	# 第二步: 只按 PNG 创建资源条目.
	# 角色资源以 `{character_id}.png` 为主导; 同 ID `.tpsheet` 或 offsets 不会反向创建 entry.
	for current_file_name in file_names:
		if not current_file_name.ends_with(".png"):
			continue

		var png_character_id := AssetParse.id_from_suffix(current_file_name, ".png")
	...
	var offsets := {}
	var offsets_path := Constants.CHARACTER_OFFSETS_PATH
	...
	if offsets_file != null:
		...
		if parsed_offsets is Dictionary:
			for character_key in parsed_offsets.keys():
				var raw_offsets = parsed_offsets[character_key]
				if not (raw_offsets is Dictionary):
					push_error("角色总偏移配置条目不是对象: %s character=%s" % [offsets_path, str(character_key)])
					continue

				var offset_character_id := int(character_key)
				var frame_offsets := FrameOffsets.new()
				frame_offsets.id = offset_character_id
				for offset_frame_key in (raw_offsets as Dictionary).keys():
					...
					if offset_data is Array and offset_data.size() == 2:
						...
						push_error("角色偏移帧格式非法, 必须是 [x, y] 两项数组: frame=%s" % str(offset_frame_key))
				offsets[offset_character_id] = frame_offsets
		else:
			push_error("角色总偏移配置不是有效 JSON: %s" % offsets_path)

	# 第四步: 为已有 PNG entry 绑定同 ID `.tpsheet`.
	# 如果 `.tpsheet` 没有对应 PNG, 直接跳过; 后续 character.yaml 引用该角色时会按缺 PNG 报错.
	for current_file_name in file_names:
