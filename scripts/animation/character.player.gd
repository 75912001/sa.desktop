...
func play_character(character_id: int, weapon: int, direction: int, action: int, target_anchor_position: Vector2) -> bool:
    ...
    assert(direction != GPB.AssetDirection.AssetDirection_Unknow, "角色动画显示方向未知: character=%d direction=%d" % [character_id, direction])
    assert(weapon != GPB.CharacterWeaponType.CharacterWeaponType_Unknow, "角色武器类型未知: character=%d weapon=%d" % [character_id, weapon])
    assert(action != GPB.CharacterAction.CharacterAction_Unknow, "角色动画动作未知: character=%d action=%d" % [character_id, action])
    ...
    assert(entry != null, "角色配置不存在: character=%d" % character_id)
