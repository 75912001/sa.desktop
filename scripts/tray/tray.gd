...
# 顺序上先缩放和移动, 再设置透明度和穿透; 如果 `设置... -> 隐藏石器` 已勾选, 最后再隐藏窗口.
# 这样可以保证用户从设置窗口取消隐藏时, 看到的是已经恢复到上次配置的窗口状态.
func _apply_window_config(window_controller: WindowController) -> void:
    ...
    var login := setting["login"] as Dictionary
    if bool(login["hide_stoneage"]):
...
# 调试边框状态已经由 OptionsDialogController 写入 tray.yaml, 这里不重复写配置.
